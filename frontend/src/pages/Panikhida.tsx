...
export default function Panikhida() {
  usePageTitle('Panikhida')
  ...
  // Filter to Panikhida-tagged pieces (the dual-service-tag pattern from
  // TAGS_REF.md means scores can also carry service:funeral — we don't care
  // here, we just need service:panikhida present), then sort by liturgical
  // order with a title-tiebreaker.
  const items = scores
    .filter((s) => s.tags.includes('service:panikhida'))
    .sort((a, b) => {
    ...
    return (
      <article>
        ...
        <p style={{ color: 'var(--text)', fontStyle: 'italic' }}>
          No hay piezas etiquetadas con <code>service:panikhida</code> todavía.
        </p>
  ...
  return (
    <article>
      ...
      <p style={{ color: 'var(--text)' }}>
        Servicio de conmemoración por los difuntos. Piezas en orden litúrgico.
      </p>
